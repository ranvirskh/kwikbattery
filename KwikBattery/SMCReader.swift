...
//
//  Keys used (Apple silicon laptops):
...
final class SMCReader {
    ...
    func snapshot(referenceAmps: Double?) -> Snapshot {
        var s = Snapshot()
        ...
        let voltageCandidates = readIntegerCandidates("B0AV").map { Double($0) / 1000.0 }
        s.batteryVoltage = voltageCandidates.first { (5.5...20.0).contains($0) }
    ...
    private func readIntegerCandidates(_ key: String) -> [Int] {
        ...
        switch raw.type {
        case "ui16":
            return [Int(le), Int(be)]
        case "si16":
            return [Int(Int16(bitPattern: le)), Int(Int16(bitPattern: be))]
        case "ui32", "si32":
    ...
    func diagnosticReport() -> String {
        ...
        for key in ["PSTR", "PDTR", "PPBR", "VD0R", "ID0R", "B0AV", "B0AC", "B0FC", "B0RM", "TB0T"] {
